...
⟨compact_iff_ultrafilter_le_nhds.mpr $ assume f uf _,
   ⟨_, trivial, (ultrafilter_converges_iff ⟨f, uf⟩ _).mpr rfl⟩⟩

instance ultrafilter.t2_space : t2_space (ultrafilter α) :=
t2_iff_ultrafilter.mpr $ assume f x y u fx fy,
  have hx : x = mjoin ⟨f, u⟩, from (ultrafilter_converges_iff ⟨f, u⟩ x).mp fx,
  have hy : y = mjoin ⟨f, u⟩, from (ultrafilter_converges_iff ⟨f, u⟩ y).mp fy,
  hx.trans hy.symm
...
begin
  ...
  refine dense_embedding.mk' pure continuous_top _ ultrafilter_pure_injective _,
  { assume x,
    rw mem_closure_iff_nhds,
    intros n hn,
    rcases (mem_nhds_of_is_topological_basis ultrafilter_basis_is_basis).mp hn
      with ⟨_, ⟨s, rfl⟩, xt, tn⟩,
    dsimp at xt,
    have : s ≠ ∅,
    { intro hs, rw [hs, empty_in_sets_eq_bot] at xt, exact absurd xt x.property.1 },
    rcases exists_mem_of_ne_empty this with a,
    apply @ne_empty_of_mem _ _ (pure a : ultrafilter α),
    split,
    { apply tn, exact mem_pure_sets.mpr h },
    { apply mem_range_self } },
  { intros a s as, exact
      ⟨{u | s ∈ u.val.sets},
       mem_nhds_sets
         (is_open_of_is_topological_basis ultrafilter_basis_is_basis ⟨s, rfl⟩)
         (mem_pure_sets.mpr (mem_of_nhds as)),
...
⟨assume h, begin
   ...
   have t : b'.val ≤ nhds b,
     from (ultrafilter_converges_iff b' _).mpr (by exact (bind_pure _).symm),
   rw ←h,
